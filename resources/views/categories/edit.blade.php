<x-layouts.app>
    <div class="py-8 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto font-sans text-slate-800">
        
        <div class="mb-6">
            <a href="{{ route('categories.index') }}" class="inline-flex items-center text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
                &larr; Batal & Kembali
            </a>
        </div>

        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
            <h1 class="text-2xl font-bold mb-6 text-slate-900">Edit Kategori</h1>
            ...
            <form action="{{ route('categories.update', $category->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full border border-slate-200 bg-slate-50 p-3 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            <div class="p-1 border border-slate-200 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors">
                                <!-- Value warna diambil dari database -->
                    ...
                    <div>
                        ...
                        <!-- Value ikon diambil dari database -->
                        ...
                        <div class="bg-slate-50 border border-slate-200 p-4 rounded-xl inline-block">
                            <div class="grid grid-cols-5 gap-3">
                                @php
                                    ...
                                    // Jika icon user tidak ada di daftar default, masukkan ke urutan pertama
                                ...
                                @foreach($emojis as $emoji)
                                    <button type="button" 
                                        class="emoji-btn w-12 h-12 flex items-center justify-center text-2xl rounded-xl border-2 transition-all duration-200 focus:outline-none hover:scale-105 {{ $currentIcon == $emoji ? 'bg-[#EEF2FF] border-[#6366F1] shadow-sm scale-105' : 'bg-transparent border-transparent hover:bg-slate-200' }}" 
                                        data-emoji="{{ $emoji }}">
                                        {{ $emoji }}
                                    </button>
                                @endforeach
                ...
                <button type="submit" class="w-full bg-slate-900 text-white py-3.5 rounded-xl font-bold hover:bg-indigo-600 transition-all duration-300 shadow-md flex items-center justify-center mt-6">
                    Simpan Perubahan
                </button>
            </form>
        </div>
